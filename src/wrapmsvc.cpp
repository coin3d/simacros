 ...
 *
 * - Check to see if compiling using the absolute path for the source
 *   file will do any difference with regards to getting the debugger
 *   to locate the Coin source files.
 */
...
//      (void)fprintf(stdout, "** ADD-TO-LIST '%s'\n", add->c_str());
  ...
  std::string commonArgs(void)
  {
    ...
    if (this->alternative_c_lib.length() > 0) {
      arg += " " + this->alternative_c_lib;
    }
...
main(int argc, char ** argv)
{
  ...
  for (int i=1; i < argc; i++) {
    ...
    // filename suffix detections).
      ...
      tool = &linker;
      linker.objname = winpath(arg.substr(5));
    }
    ...
    // Compile for specific hardware.
      ...
      if (intelcpu.compare(prefix)==0) { intelcpu.erase(0, strlen(prefix)); }
      const char * cpuconv[] = {
        "i386", "G3",
        "i486", "G4",
        "pentium", "G5",
    ...
    // Alternative C-libraries.
      ...
      compiler.alternative_c_lib = arg;
    ...
    // Name the objectfile / executable.
      ...
      std::string objname = winpath(argv[++i]);
    ...
    // Name the .pdb debug file.
      ...
      tool = &compiler;
      tool->pdbname = winpath(arg.substr(3));
    }
